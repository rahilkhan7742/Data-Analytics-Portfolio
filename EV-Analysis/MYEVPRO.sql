...
-- Total EV Registration Count

SELECT COUNT(*) AS total_ev_registration FROM electric_vehicle_population;
...
-- Clean Alternative Fuel Vehicle(CAFV) Eligibility Breakdown by Top Brands
SELECT 
    Make,
...
HAVING COUNT(*) > 5000;
   ...
   -- total vehicales and average battery range for brands with over 5000 sales                    
  ...
  -- total vehicle volume per brand along with its overall market share percentage 
   WITH BrandTotal AS (
   ...
   -- track yearly EV adoptions alongside previous year totals to mesure Year-over-Year Growth
